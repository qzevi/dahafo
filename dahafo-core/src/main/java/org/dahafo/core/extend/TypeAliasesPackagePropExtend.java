...
package org.dahafo.core.extend;
...
public class TypeAliasesPackagePropExtend implements FactoryBean<String> {
     ...
     * 详细描述：该方法在加载配置时自动调用，�?�过对entity的包路径进行解析，把�?有解析到的实体路径拼接成字符串返回给mybatis�?<br/>
     * 使用方式：该方法无需调用，只�?要在xml中配置mybatis的相关配置，具体参照applicationContext-dao-base.xml文件�?
    ...
    public String getObject() throws Exception {
        ...
        Assert.notNull(this.typeAliasesPackages);
        try {
...
//                    path = path.replaceAll("/", ".");
            ...
            log.debug("method getObject() return :{}",resultStr);
            return resultStr;
        } catch (IOException e) {
    ...
    /**
     * typeAliasesPackages变量的get方法，为了获取配置的�?有路径�??
     * @return typeAliasesPackages配置的所有路�?
     */
